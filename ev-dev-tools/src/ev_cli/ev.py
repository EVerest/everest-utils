...
def generate_tmpl_data_for_if(interface, if_def, type_file):
    ...
    duplicates = {}
    helpers.rename_duplicates(enums, duplicates, 'Enum')
    helpers.rename_duplicates(types, duplicates, 'Type')

    # check if any cmds arguments or result types have to be re-named
    helpers.rename_cmd_args_and_results(cmds, enums, types)
...
def types_genhdr(args):
    print(f"Generating global type headers.")
    output_dir = Path(args.output_dir).resolve() if args.output_dir else everest_dir / \
